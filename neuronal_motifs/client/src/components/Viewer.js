...
import {Color, getTranslationVectors, mapQueryResult} from "../utils/rendering";
import BasicMenu from "./ContextMenu";
import axios from "axios";
import {getAuthToken} from "../utils/authentication";
import _ from "lodash";
import {RoiMap} from "../config/RoiMap";

const roiRegEx = new RegExp(/^([a-zA-Z\']+)(?:|\d|\-|)/g);
...
function Viewer() {
    ...
    function parseRoiInfo(rawRoiInfo) {
        const parsedRoiInfo = JSON.parse(rawRoiInfo);
        const regExMatches = Object.keys(parsedRoiInfo).map((key) => {
            let match = String(key).match(roiRegEx)
            let matchString = _.first(match)
            if (!matchString) {
                console.log('Regex Missed', key)
            }
            let mapVal = RoiMap[matchString];
            if (!mapVal) console.log('Map Missed', matchString)
            return RoiMap[matchString];
        });
        return _.countBy(regExMatches);
    }
    ...
    function onSynapseSuggestionEvent(
        ...
        if (context.neighborhoodQuery) {
            let neighborhoodQuery = context.neighborhoodQuery;
            neighborhoodQuery.results.map((result) => {
                // iterate over all elements in result
                const roiMatches = Object.entries(result).map(([label, neuron]) => {
                    return parseRoiInfo(neuron.roiInfo)
                })
                console.log(roiMatches, roiMatches)
            })
        }
    }
