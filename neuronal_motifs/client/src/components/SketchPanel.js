...
function SketchPanel() {
  ...
  const getMotifCount = async (motif) => {
    let url = `${process.env.REACT_APP_PROTOCOL}://${process.env.REACT_APP_API_URL}/count/motif=${motif}`;
    return (await axios.get(url)).data;
  };

  const getRelativeMotifCount = async (motif) => {
    // get request to backend to get motif count
    let url = `${process.env.REACT_APP_PROTOCOL}://${process.env.REACT_APP_API_URL}/rel_count/motif=${motif}`;
    return (await axios.get(url)).data;
  };
