...
function SketchPanel() {
    ...
    const getPositionTransformedData = (data) => {
        ...
        let newData = {
            ...data, "nodes": newNodes, "dimension": canvasDimension
        };
        return newData;
    };
    ...
    const placeCircle = (circle, letter, properties = null, tree = null) => {
        ...
        let label = new paper.PointText({
            point: textPoint, justification: "center", fillColor: "white", font: "Roboto", fontSize: 20,
        });
        ...
        if (tree !== null) {
            ...
            setNodes((nodes) => [...nodes, {
                circle: circle, label: letter, properties: properties, type: "node", circleGroup: circleGroup,
            },]);
        }
    };
    ...
    const createEdge = (fromNode, toNode, edgeLine, nodeIndices, properties = null, tree = null, propertyLabel = null) => {
        let edgeObj = {
            indices: nodeIndices, toNode: toNode, fromNode: fromNode, edgeLine: edgeLine,
        };
    ...
    const addEdgePropertyLabel = (e) => {
        ...
        let propertyLabel = new paper.PointText({
            point: topPoint, justification: "center", fillColor: "black", font: "Roboto", fontSize: 14,
        });
    ...
    useEffect(() => {
        if (context.selectedSketchElement) {
            ...
            if (paperElement && position) {
                setPopperLocation({
                    top: position.y + boundingRect.top + 30, left: position.x + boundingRect.left - 30,
                });
            }
    ...
    const getEncodedMotif = (nodes, edges) => {
        let encodedNodes = nodes.map((n, i) => {
            return {
                label: n.label, properties: n.properties, index: i, position: n.circle.position, tree: n.tree,
            };
        });
        let encodedEdges = edges.map((e, i) => {
            return {
                label: e.label, properties: e.properties, index: i, indices: e.indices, tree: e.tree,
            };
        });
        let sketchPanelDim = {
            width: canvasDimension.width, height: canvasDimension.height
        }
        ...
        <Grid container className="canvas-wrapper" spacing={0}>
            ...
            <Grid item xs={10} style={{height: "inherit"}}>
                ...
                >
                    ...
                    {showPopper && popperLocation && context.selectedSketchElement && (<Popover
                  ...
                  <span
                      style={{
                          paddingLeft: 10, fontWeight: "bold", color: "#454545",
                      }}
                  >
